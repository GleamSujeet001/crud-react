...
function SignupList({setUserImage }) {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/get-user-data')
      .then((response) => {
        setRows(response.data);
           // Assuming the logged-in user is the first user in the data array:
           const currentUser = response.data[0];  // Update this logic as needed
           if (currentUser) {
             setUserImage(currentUser.imageUrl ? `${currentUser.imageUrl}?t=${new Date().getTime()}` : `http://localhost:3000/${currentUser.image}?t=${new Date().getTime()}`);
           }
      })
      .catch((error) => {
...
const handleEditSubmit = () => {
  if (selectedRow) {
    ...
    axios.put(`http://localhost:3000/update-user-data/${selectedRow._id}`, formData, {
      headers: {
 ...
 const handleClickOpen = (id) => {
  ...
  const handleConfirmDelete = () => {
    if (selectedId) {
      axios.delete(`http://localhost:3000/delete-user-data/${selectedId}`)
        .then(() => {
      ...
      <Table  sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        <TableBody>
         ...
         sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
              ...
              <Avatar alt={row.name} src={row.imageUrl ? `${row.imageUrl}?t=${new Date().getTime()}` : `http://localhost:3000/${row.image}?t=${new Date().getTime()}`} />

            </TableCell>
              <TableCell >
              <IconButton aria-label="delete" style={{ color: 'blue' }} onClick={() => handleEdit(row)}>
              <EditIcon />
              </IconButton>
